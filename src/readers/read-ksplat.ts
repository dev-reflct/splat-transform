...
export const readKsplatFromBuffer = async (buffer: ArrayBuffer): Promise<KsplatFileData> => {
    ...
    if (buffer.byteLength < MAIN_HEADER_SIZE) {
        throw new Error('File too small to be valid .ksplat format');
    }

    // Parse main header
    const mainHeader = new DataView(buffer, 0, MAIN_HEADER_SIZE);
    ...
    for (let sectionIdx = 0; sectionIdx < maxSections; sectionIdx++) {
        ...
        const sectionHeader = new DataView(buffer, sectionHeaderOffset, SECTION_HEADER_SIZE);
    ...
    for (let sectionIdx = 0; sectionIdx < maxSections; sectionIdx++) {
        ...
        const bucketCenters = new Float32Array(buffer, bucketCentersOffset, bucketCount * 3);
        ...
        const splatData = new DataView(buffer, splatDataOffset, sectionDataSize);
...
/**
 * Read KSPLAT data from a File
 * @param file - The File containing KSPLAT data
 * @returns Promise that resolves to KsplatFileData
 */
